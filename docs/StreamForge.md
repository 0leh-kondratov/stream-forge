Проект технического документа (Версия 2.0)СодержаниеЧасть I: Введение и бизнес-контекстГлава 1: Общее описание проекта StreamForge1.1. Проблема: Сложность и волатильность потоков крипто-данных1.2. Видение: Создание интеллектуальной, событийно-управляемой аналитической платформы1.3. Ключевые цели и польза для бизнеса1.4. Основные сценарии использования
...
Для проверки работоспособности системы и тестирования сквозного прохождения команд и событий через Kafka используются `dummy-service` и `debug_producer.py`.

**1. `dummy-service`: Тестовый микросервис**

`dummy-service` — это тестовый микросервис, который может имитировать поведение других сервисов, получать команды из Kafka (`queue-control`) и отправлять события (`queue-events`). Он используется для симуляции, тестирования связности Kafka и отладки.

*   **Запуск:** Запустите `dummy-service` в Kubernetes как `Job` или `Pod`, передав ему необходимые переменные окружения.
*   **Подробнее:** См. `services/dummy-service/README.md`.
...
*   **Отправка тестовой команды (ping/pong):**
...
*   **Отправка команды остановки (stop):**
    ...
    Эта команда отправит сигнал `stop` целевому сервису, который должен корректно завершить свою работу.
...
Для отладки и взаимодействия с кластером Kubernetes в проекте предусмотрены две основные среды:
...
*   **Dev-контейнер:** Представляет собой Docker-образ с широким набором инструментов для разработки и отладки, включая `kubectl`, `helm`, `kafkacat` и `python`. Этот образ можно использовать для запуска временных подов в Kubernetes, которые служат в качестве полноценной интерактивной среды для отладки микросервисов.
