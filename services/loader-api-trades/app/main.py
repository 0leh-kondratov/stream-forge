...
app = FastAPI(title="Loader API Trades Service")
...
async def main():
    ...
    logger.info(f"🚀 Starting Loader API Trades: {config.QUEUE_ID} -> {config.KAFKA_TOPIC}")
    await telemetry.send_status_update(status="started", message="Loader API Trades started")
    ...
    logger.info("✅ Loader API Trades shutdown complete.")
...
if __name__ == "__main__":
    try:
        ...
        logger.exception(f"❌ Fatal error in Loader API Trades: {e}")