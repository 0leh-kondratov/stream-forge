...
app = FastAPI(title="Arango Trades Service")
...
async def main():
    ...
    logger.info(f"🚀 Starting Arango Trades: {config.QUEUE_ID} -> {config.COLLECTION_NAME}")
    await telemetry.send_status_update(status="started", message="Arango Trades consumer started")
    ...
    logger.info("✅ Arango Trades shutdown complete.")
...
if __name__ == "__main__":
    try:
        ...
        logger.exception(f"❌ Fatal error in Arango Trades: {e}")
