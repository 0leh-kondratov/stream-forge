...
app = FastAPI(title="Arango Graph Prep Service")
...
async def main():
    ...
    logger.info(f"🚀 Starting Arango Graph Prep: {config.QUEUE_ID} -> {config.COLLECTION_NAME}")
    await telemetry.send_status_update(status="started", message="Arango Graph Prep consumer started")
    ...
    logger.info("✅ Arango Graph Prep shutdown complete.")
...
if __name__ == "__main__":
    try:
        ...
        logger.exception(f"❌ Fatal error in Arango Graph Prep: {e}")
