...
app = FastAPI(title="Loader WS Trades Service")
...
async def main():
    ...
    logger.info(f"🚀 Starting Loader WS Trades: {config.QUEUE_ID} -> {config.KAFKA_TOPIC}")
    await telemetry.send_status_update(status="started", message="Loader WS Trades started")
    ...
    logger.info("✅ Loader WS Trades shutdown complete.")
...
if __name__ == "__main__":
    try:
        ...
        logger.exception(f"❌ Fatal error in Loader WS Trades: {e}")
